...
class HTML_QuickForm_Renderer
{
    ...
    */
    ...
    /**
     * Called to check if list of Array keys are valid
     *
     * @param  string $keys
     */
    public static function validateSmartyKeys($keys) {
      if (strstr($keys, '][')) {
        $tokens = explode('[', str_replace(']', '', $keys));
      }
      else {
        $tokens = (array) str_replace([']', '['], '', $keys);
      }
      $inValidTokens = array_filter($tokens,
        function($token) {
          return !(CRM_Utils_Type::escape($token, 'Int', FALSE) || CRM_Utils_Rule::xssString($token));
      });
      if (!empty($inValidTokens)) {
        return PEAR::raiseError(null, QUICKFORM_ERROR, null, E_USER_WARNING, "Invalid array key found", 'HTML_QuickForm_Error', true);
      }
    }
